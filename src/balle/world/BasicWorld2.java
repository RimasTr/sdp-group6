...
public class BasicWorld2 extends AbstractWorld {
	...
	public void updateScaled2(Coord ourPosition, Orientation ourOrientation,
			Coord theirPosition, Orientation theirOrientation,
			Coord ballPosition, long timestamp) {
		...
		// change in time
		// Hopefully that does not cause issues with EmptySnapshot and
		// currentTimeMilis()
		...
		if (theirOrientation == null) {
			theirOrientation = prev.getOpponent().getOrientation();
		}

		Robot ours = prev.getBalle();
		Robot theirs = prev.getOpponent();
		Ball ball = prev.getBall();

		ours.update(ourPosition, ourOrientation, deltaT);
		theirs.update(theirPosition, theirOrientation, deltaT);
		ball.update(ballPosition, deltaT);

		Snapshot nextSnapshot = new Snapshot(this, theirs, ours, ball,
				timestamp);
		prevRaw = nextSnapshot;
